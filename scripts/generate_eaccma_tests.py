#!/usr/bin/env python3
"""Generate EACCMA test files from JSON"""
    ...
    <div id="start-screen" class="text-center py-12">
      <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-8 max-w-2xl mx-auto">
        ...
        <p class="text-slate-600 dark:text-slate-300 mb-6">
          This test contains {question_count} multiple-choice questions on EACCMA.
          You have {time_limit} minutes to complete it.
        </p>
...
def main():
    print("Loading EACCMA tests from JSON...")
    test_data = load_tests('eaccma/eaccma_12_main_tests.json')
    
    # Create tests directory
    tests_dir = 'flashcards/eaccma/tests'
    os.makedirs(tests_dir, exist_ok=True)
    
    print(f"Generating {test_data['total_tests']} tests...")
    ...
    for i in range(1, test_data['total_tests'] + 1):
        ...
        print(f"  ✓ Test {i}: {test['title']} ({test['question_count']} questions, {time_limit} min)")
    
    print(f"\nDone! Created {test_data['total_tests']} tests in {tests_dir}/")
